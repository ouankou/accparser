#!/bin/bash
#
# extract_openacc_vv_pragmas.sh - Extract OpenACC pragmas from OpenACCV-V suite
#
...
#   LLVM_VERSION=<LLVM_VERSION> CC=clang-<LLVM_VERSION> CXX=clang++-<LLVM_VERSION> FC=flang-<LLVM_VERSION> ./extract_openacc_vv_pragmas.sh
#
# The script will:
# 1. Wipe the tests/openacc_vv/ directory completely
# 2. Clone/update the OpenACCV-V repository
# 3. Extract pragmas from all C/C++/Fortran files using LLVM preprocessors
# 4. Save each pragma to a separate file in tests/openacc_vv/
# 5. Update this script's header with the OpenACCV-V commit hash and date
# 6. Report the total number of pragmas extracted
#
# Extracted from OpenACCV-V repository:
# Commit: b7c28b9cddbeb38fa496df9e3e88943d8ff86dae
# Date:   2025-11-03 11:18:32
#
# Last extraction: 2025-11-05 22:53:50
#
...
TESTS_DIR="Tests"
OUTPUT_DIR="tests/openacc_vv"
REQUIRED_MIN_PRAGMAS=5783
...
echo "========================================="
echo "  OpenACCV-V Pragma Extraction"
echo "========================================="
echo ""
...
# Clone or update OpenACCV-V repository
if [ ! -d "$REPO_PATH" ]; then
    echo "Cloning OpenACCV-V repository..."
    ...
    echo "Updating existing OpenACCV-V repository..."
    cd "$REPO_PATH"
    git fetch origin || {
...
echo "Reading OpenACCV-V repository information..."
cd "$REPO_PATH"
...
for file in "${source_files[@]}"; do
    ...
    if [ $is_fortran -eq 1 ]; then
        # Fortran file
        preprocessed=$("$REQUIRED_FC" -E -P "$file" "${include_flags[@]}" 2>/dev/null || true)
        ...
        preprocessed=$("$preprocessor" -E -P -CC "$file" "${include_flags[@]}" 2>/dev/null || true)
